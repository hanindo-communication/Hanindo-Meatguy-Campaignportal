...
2. **Deploy:** Host Node (PM2, VPS, dll.): set env `SESSION_SECRET` dan `PORTAL_PASSWORD`, jalankan `node server/index.js`. **Vercel:** project ini memakai [`vercel.json`](vercel.json) + [`api/index.js`](api/index.js) (Express sebagai serverless). Di dashboard Vercel set **Environment Variables** `SESSION_SECRET` (≥16 karakter) dan `PORTAL_PASSWORD`, lalu deploy ulang.
